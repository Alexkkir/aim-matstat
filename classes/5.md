...
* метод, позволяющий улучшить нахавляву оценку
...
$$
\widehat{Bias(\hat \theta_n)} = (n - 1) \Big( {1 \over n} \sum_i^n \hat \theta_{(-i)} - \hat \theta_n)
$$
...
Можно придумать тест, у которого $\a$ фиксирована, а кривая оптимальна?

> Да, это называется UMP tests (uniformly most powerful)
...
* не является тестов в строгом смысле этого слова
...
$$
Рандомизированный тест: вводим функция $d$:
$$
...
У этой теоремы есть предположения, но опустим их - условия регулряности. Но они почти всегда выполнены.
...
* поговоририли про тесты, рандомизированные тесты
